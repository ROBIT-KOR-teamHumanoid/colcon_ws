#include "../include/robocup_master25/robocup_master25_node.hpp"

#include <rclcpp/rclcpp.hpp>

namespace robocup_master25 {
RobocupMaster25Node::RobocupMaster25Node(const rclcpp::NodeOptions &options)
    : rclcpp_lifecycle::LifecycleNode("robocup_master25_node", options) {
...
RobocupMaster25Node::on_configure(
    const rclcpp_lifecycle::State &previous_state) {
  RCLCPP_INFO(get_logger(), "Configuring node");
  get_parameters();

  ik_pub =
      create_publisher<humanoid_interfaces::msg::Master2IkMsg>("master2ik", 10);
  vision_pub = create_publisher<humanoid_interfaces::msg::Master2vision25>(
      "master2vision", 10);
  gamecontrol_pub =
      create_publisher<humanoid_interfaces::msg::Gamecontrolreturndata>(
          "gamecontrolreturndata", 10);
  local_pub = create_publisher<humanoid_interfaces::msg::Master2localization25>(
      "master2local", 10);
  udp_pub =
      create_publisher<humanoid_interfaces::msg::Master2udp>("master2udp", 10);
  ...
  imu_sub = create_subscription<humanoid_interfaces::msg::ImuMsg>(
      ...
      std::bind(&RobocupMaster25Node::imuCallback, this,
                std::placeholders::_1));
  vision_sub = create_subscription<humanoid_interfaces::msg::Robocupvision25>(
      "vision", 10,
      std::bind(&RobocupMaster25Node::visionCallback, this,
                std::placeholders::_1));
  ik_sub = create_subscription<humanoid_interfaces::msg::IkEndMsg>(
      "ik", 10,
      std::bind(&RobocupMaster25Node::ikCallback, this, std::placeholders::_1));
  gamecontrol_sub =
      create_subscription<humanoid_interfaces::msg::Gamecontroldata>(
          "gamecontroldata", 10,
          std::bind(&RobocupMaster25Node::gamecontrolCallback, this,
                    std::placeholders::_1));
  local_sub =
      create_subscription<humanoid_interfaces::msg::Robocuplocalization25>(
          "localization", 10,
          std::bind(&RobocupMaster25Node::localCallback, this,
                    std::placeholders::_1));
  udp_sub = create_subscription<humanoid_interfaces::msg::Udp2master>(
      "udp", 10,
      std::bind(&RobocupMaster25Node::udpCallback, this,
                std::placeholders::_1));
  motionSub = create_subscription<humanoid_interfaces::msg::MotionOperator>(
      "motion_end", 10,
      std::bind(&RobocupMaster25Node::motionCallback, this,
                std::placeholders::_1));
  pid_sub = create_subscription<humanoid_interfaces::msg::Pidtuning>(
      "pid", 10,
      std::bind(&RobocupMaster25Node::pidCallback, this,
                std::placeholders::_1));

  try {
    master = std::make_shared<robocup_master25::MasterRcko>();
    master->kp = kp_;
    master->kd = kd_;
    master->ki = ki_;
...
rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn
RobocupMaster25Node::on_activate(
    const rclcpp_lifecycle::State &previous_state) {
  ...
  try {
    timer = create_wall_timer(
        ...
        std::bind(&RobocupMaster25Node::robocup_master, this));
...
rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn
RobocupMaster25Node::on_deactivate(
    const rclcpp_lifecycle::State &previous_state) {
...
rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn
RobocupMaster25Node::on_cleanup(const rclcpp_lifecycle::State &previous_state) {
...
rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn
RobocupMaster25Node::on_shutdown(
    const rclcpp_lifecycle::State &previous_state) {
...
RobocupMaster25Node::on_parameter_change(
...
void RobocupMaster25Node::init_parameters() {
...
void RobocupMaster25Node::get_parameters() {
...
void RobocupMaster25Node::ikCallback(
    const humanoid_interfaces::msg::IkEndMsg::SharedPtr msg) {
  master->ikEnd.ikend = msg->ikend;
}

void RobocupMaster25Node::gamecontrolCallback(
    const humanoid_interfaces::msg::Gamecontroldata::SharedPtr msg) {
  master->gameControlData.robotnum = msg->robotnum;
...
void RobocupMaster25Node::localCallback(
    const humanoid_interfaces::msg::Robocuplocalization25::SharedPtr msg) {
  master->local.robot_x = msg->robot_x;
...
void RobocupMaster25Node::udpCallback(
    const humanoid_interfaces::msg::Udp2master::SharedPtr msg) {
  int i = static_cast<int>(msg->robotnum - 1);
...
void RobocupMaster25Node::robocup_master() {
  if (!testFlag) {
  ...
  if (!player) {
    switch (position) {
      case POSITION_GK:
        RCLCPP_INFO(get_logger(), "POSITION_GK");
        player = std::make_shared<robocup_master25::Goalkeeper>(master);
        break;
      case POSITION_FW:
        RCLCPP_INFO(get_logger(), "POSITION_FW");
        player = std::make_shared<robocup_master25::Forward>(master);
        break;
      default:
        // RCLCPP_INFO(get_logger(), "NO POSITION SET");
        break;
    }
  }
  ...
  if (player) {
    ...
    vision_pub->publish(master->master2vision);
    gamecontrol_pub->publish(master->gameControlReturnData);
    local_pub->publish(master->master2local);
    udp_pub->publish(master->master2udp);
    if (player->publish_motion_flag) {
      motionPub->publish(master->motion);
...
int main(int argc, char **argv) {
  ...
  auto node = std::make_shared<robocup_master25::RobocupMaster25Node>();
